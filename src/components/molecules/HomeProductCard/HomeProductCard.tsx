...
export const HomeProductCard = ({
  ...
  const [currentModalProductIndex, setCurrentModalProductIndex] =
  ...
  const originalTotalInventory =
    api_product?.variants?.reduce((sum, variant) => {
  ...
  const totalInventory = isHydrated
    ? api_product?.variants?.reduce((sum, variant) => {
        ...
        const adjustedInventory = getAdjustedInventory(
          variant.id,
          originalInventory
        )
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    console.log(
      ...
      api_product.id,
      api_product.variants?.[0]?.id,
      totalInventory
    )
    if (isAddingToCart || !api_product.variants?.[0]?.id || totalInventory <= 0)
      return
          ...
          <AddVariantSheet
            product={
              allProducts.length > 0
                ? allProducts[currentModalProductIndex]
                : api_product
            }
            cardPos={cardPos}
